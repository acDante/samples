...
double tab_joint_right[3][8] = {{1.2,0,0.30,0,0,0,0,0},{1.78,0,0.30,0,0,0,0},{2.3,0,0.30,0,0,0,0,0}};
...
void UserController::onInit(InitEvent &evt)
{
   ...
   restart_on = false;
    ...
    my->setJointAngle("LARM_JOINT2",-1.57);
 ...
 // printf("all_msg=\n%s\n",all_msg);
  ...
  headss.assign(ss, strPos1, strPos2-strPos1);
   ...
   if (strcmp(all_msg,"go") == 0 ) {
    ...
    //	sendMsg("VoiceReco_Service","Cancel the action ..");
    	//sleep(4);
    	sendMsg("moderator_0","switch_range");
...
else if (strcmp(all_msg,"Pet") == 0 ) {

moveRightArm(0);
}

else if (strcmp(all_msg,"Mug") == 0 ) {

moveRightArm(1);
}
else if (strcmp(all_msg,"Can") == 0 ) {
...
{
  //自分自身の取得
  SimObj *my = this->getObj("robot_000");
  ...
  if(headss == "ORS_DATA"){
    ...
    dQuaternion tmpQ3;
    dQMultiply1(tmpQ3, bodyQ, tmpQ2);
    my->setJointAngle("NECK_JOINT0",-pyaw);
    my->setJointAngle("NECK_JOINT1",-pitch);
   // my->setJointAngle("NECK_JOINT2",-proll);
   // my->setJointQuaternion("NECK_JOINT0", tmpQ3[0], tmpQ3[1], tmpQ3[2], tmpQ3[3]);
  }
}
